#!/usr/bin/env python3
"""Verify cached Lake dependency sources against the frozen manifest.

This is the source half of COSMO Phase B1 / OPT-LEAN-001.  Compiled
...
SCHEMA = "COSMO-LEAN-SOURCE-RECEIPT-1"
...
def assert_sanitized_git_metadata(path: Path, package: str) -> None:
    ...
    for forbidden in (path / ".git" / "hooks", path / ".git" / "config.worktree"):
        if forbidden.is_symlink() or forbidden.exists():
...
def validated_manifest_url(raw: Any, package: str) -> str:
    ...
    if package == "mathlib":
        ...
        expected = TOP_LEVEL_MATHLIB_URL[:-4]
...
def verify_commit_tree(path: Path, package: str, revision: str) -> str:
    ...
    if generated_lake.exists() or generated_lake.is_symlink():
        raise SystemExit(
            f"generated package Lake state must be purged before source verification: {generated_lake}"
        )
...
def load_manifest(path: Path, expected_sha256: str) -> tuple[str, list[dict[str, Any]]]:
    ...
    if any(isinstance(package, dict) and package.get("type") == "path" for package in packages):
        raise SystemExit("protected COSMO B1 baseline forbids local path dependencies")
...
def main() -> None:
    ...
    print(
        "COSMO dependency-source-state verified "
